...
public class PageMaker {
	...
	private int page;// 첫페이지를 표시하기 위함, 페이지 번호를 나타낸다
	private int perPageNum = 10;// 한페이지에 몇개 보일지
	...
	public void setEndPage(int getlastblock, int getcurrentblock) {
		if (getlastblock == getcurrentblock) {
			this.endPage = calcpage(getTotalcount(), getPerPageNum());
		} else {
	...
	public void setCurrentblock(int page) {// 현재 페이지 블록의 번호가 몇번인지 지정해주는 함수
...
//        ->나머지 있으면 더하기 1한다. -> 이 결과를 현재 블록 번호라고 부른다. cblock
	...
	public void setLastblock(int totalcount) {// 마지막 페이지 블록의 번호가 뭔지 지정해주는 함수
		// 전체 글 개수를 사용해서 지정한다
		this.lastblock = totalcount / (5 * this.perPageNum);
		if (totalcount % (5 * this.perPageNum) > 0) {
			this.lastblock++;
		}
...
//        ->나머지 있으면 더하기 1한다. -> 이 결과를 마지막 블록 번호라고 부른다. last block
	...
	public int calcpage(int totalcount, int perPageNum) {// 전체 몇페이지까지 있을지 함수로 계산한다
		int totalpage = totalcount / perPageNum;// 전체 게시물 수를 한 페이지당 몇개 보이는지로 나눈다
		if (totalcount % perPageNum > 0) {// 그리고 나머지가 있다면 1을 더해서 한 페이지 증가 시킨다
			totalpage++;
		}
